...
## Поточний стан (оновлено 2026-09-18)

**MVP, Фаза 2 та Фаза 3 реалізовані.** Бекенд, бот, інфраструктура та всі екрани Mini App
(включно з мультилокаційним кошиком, чекаутом, історією замовлень, верифікацією та адмін-панеллю) працюють.
...
- Бот: `/start` із кнопкою Mini App + постійна Menu Button + інтерактивна модерація замовлень
  (зміна статусів, коригування часу, автоматичні сповіщення клієнта) + верифікація телефону через контакт
- Mini App: екран категорій, товарів, кошик з розбиттям по закладах,
  ...
  історія замовлень із повтором у клік, профіль клієнта з бейджем верифікації, повноцінна адмін-панель
- CI: лінт (ruff), компіляція синтаксису, імпорти, конфіги, типи та збірка фронтенду, три docker-образи, пошук секретів

**Далі:** Фаза 4 — віртуальна оплата (Telegram Payments).
