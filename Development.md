...
## Поточний стан (оновлено 2026-09-15)

**MVP та Фаза 2 реалізовані.** Бекенд, бот, інфраструктура та всі екрани Mini App
(включно з мультилокаційним кошиком, чекаутом та модерацією) працюють.

**Готово:**

- Backend — 14 ендпоінтів, перевірені наскрізно
- Схема БД (`0001_init.sql` ... `0004_orders.sql`) застосована:
  `orders`, `order_groups`, `order_items`, `order_item_options`
- Дворівневі категорії з прихованими та стоп-лист (`is_available`)
- Групи опцій з безкоштовною квотою
- Бот: `/start` із кнопкою Mini App + постійна Menu Button + модерація замовлень
  з кнопками Підтвердити/Відхилити та сповіщенням клієнта
- Mini App: екран категорій, товарів, кошик з розбиттям по закладах,
  чекаут (доставка/самовивіз, оплата), екран успішного замовлення, профіль
- CI: лінт, імпорти, конфіги, типи та збірка фронтенду, три docker-образи, пошук секретів

**Далі:** Фаза 3 — історія замовлень.
