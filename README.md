...
> **Статус: Фаза 2 реалізована.** Бекенд, бот, інфраструктура, каталог, мультилокаційний кошик,
> повноцінний чекаут (доставка/самовивіз), модерація замовлень менеджером у Telegram-боті
> та профіль клієнта готові.
> Далі — історія замовлень (Фаза 3), див. [Roadmap](#roadmap).
...
- **Модерація замовлень через Telegram-бота:**
  - Миттєве надсилання картки нового замовлення менеджеру в чат (`MANAGER_CHAT_ID`)
  - Інлайн-кнопки «✅ Підтвердити» та «❌ Відхилити» з блокуванням повторних натискань
  - Автоматичне сповіщення клієнта у боті про статус замовлення та деталі доставки
- **Контакти:**
...
кошик із розділенням закладу, чекаут, екран успішного замовлення, профіль клієнта, контакти.
Нативна кнопка «Назад», тактильний відгук (Haptic), підтримка світлої й темної тем Telegram.

**Заплановано:** історія замовлень (Фаза 3), Telegram Payments (Фаза 4) — див. [Development.md](Development.md).
...
```bash
# Автоматично застосувати всі міграції (db/migrations/):
python3 scripts/migrate.py

# Наповнити початковий каталог:
...
3 | Історія замовлень, повтор у клік | 🟡 у планах (активна)
4 | Telegram Payments (онлайн-оплата) | заплановано
5 | Push-нотифікації статусів, адмін-панель | заплановано
